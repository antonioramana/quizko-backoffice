                                    ...
                                    <ul className="list-disc pl-5">
                                        {selectedQuestions.map((question,index) => (
                                            <li key={question.value} className="text-purple-500 mb-1 flex items-center justify-between">
                                                  <span>{index+1+") "} {question.label}</span> {/* Display label with points */}
                                                <span>{questions.find(q => q.id === question.value)?.point} pts</span> {/* Display points for each question */}
                                            </li>
                                        ))}
                                    </ul>
                                    <div className="mt-2 font-medium">Total des points: {totalPoints}</div> {/* Display total points */}
                                </div>
